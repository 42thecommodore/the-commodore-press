...
Every title and life also gets a page of its own, built from the same file: `…/t/forty-two/` for a title, `…/l/john-snow/` for a life. **That is the link to post or send.** It previews as the entry itself, with its own picture, and it is the page search engines find. The `#t/…` address in the library's bar previews as the front door. Every entry, in the library and on its own page, has a **Share** button in the top bar (it opens your phone's share sheet, or a card with copy, email, X and LinkedIn), **Share this line** under the line to keep, and an end-of-entry block with a **Read next** card, taken from the entry's first `across` link. Select any passage and a small **Share quote** button appears; a shared quote is always credited "The Commodore Press, on <entry>", never to the person the entry is about. None of it loads anything from the social networks: they are plain links. Readers' browsers remember which entries they finished and where they stopped, and nowhere else; the colophon says so. Nothing to maintain: the build makes these pages, and the sitemap, on every run.

The preview card for a title or a life is a picture in the entry's own colours, made by `npm run card` (it needs Google Chrome installed): titles in `assets/cards/`, lives in `assets/cards/l/` with their portrait, and the photographer's credit printed on the card when the plate's licence asks for one. When you rename an entry, rewrite a title's claim or a life's lede, replace a portrait, or add a new entry, the check says so in yellow — run `npm run card` and look at the pictures before shipping. It also measures every card and stops with a list if any text does not fit; shorten that entry's claim or lede, or ask for the type to be adjusted.
